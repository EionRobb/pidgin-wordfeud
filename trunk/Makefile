...
all:	pidgin-wordfeud.so pidgin-wordfeud.dll pidgin-wordfeud.so installers sourcepackage
...
pidgin-wordfeud.dll:	${FACEBOOK_SOURCES} #pidgin-facebookchat.res
...
pidgin-wordfeud-debug.dll:	${FACEBOOK_SOURCES} #pidgin-facebookchat.res
	${WIN32_COMPILER} ${LIBPURPLE_CFLAGS} -Wall -I. -g -pipe ${FACEBOOK_SOURCES} -o $@ -shared -mno-cygwin ${WIN32_CFLAGS} ${WIN32_LIBS}
